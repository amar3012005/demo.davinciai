"""
TTS-Sarvam Streaming Microservice FastAPI Application

WebSocket-based TTS streaming service using Sarvam AI's models.
...
from sarvam_manager import SarvamManager
...
manager: Optional[SarvamManager] = None
...
async def lifespan(app: FastAPI):
    ...
    logger.info("=" * 70)
    logger.info("🚀 Starting TTS-Sarvam Microservice")
    logger.info("=" * 70)
    
    try:
        # Load configuration
        config = SarvamConfig.from_env()
        logger.info("✅ Configuration loaded")
        
        # Initialize Sarvam manager
        manager = SarvamManager(config)
        ...
        logger.info("=" * 70)
        logger.info("✅ TTS-Sarvam microservice ready")
        logger.info(f"   Model: {config.model}")
        logger.info(f"   Speaker: {config.voice_id}")
        logger.info(f"   Sample rate: {config.sample_rate}Hz")
        logger.info(f"   Output format: {config.output_audio_codec}")
        logger.info("=" * 70)
        ...
        # Cleanup
        logger.info("Shutting down TTS-Sarvam...")
        if manager:
            await manager.close()
        ...
        logger.info("TTS-Sarvam shutdown complete")


app = FastAPI(
    title="TTS-Sarvam",
    description="Ultra-low latency Text-to-Speech streaming using Sarvam AI",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    ...
    return {
        "service": "TTS-Sarvam Streaming Service",
        "version": "1.0.0",
        "model": config.model if config else None,
        "speaker": config.voice_id if config else None,
        "endpoints": {
...
async def synthesize(request: SynthesizeRequest):
    ...
    # Send as base64 to match Cartesia signature
    return {
        "audio": base64.b64encode(audio_bytes).decode("utf-8"),
        "sample_rate": config.sample_rate,
        "format": "pcm_f32le" if config.output_audio_codec == "linear16" else config.output_audio_codec,
...
async def websocket_stream(
    ...
    - Server sends: binary audio chunk (bytes)
    ...
    # Send connection confirmation matching Cartesia structure
    await websocket.send_json({
        ...
        "sample_rate": config.sample_rate if config else 24000,
        "format": "pcm_f32le" if config and config.output_audio_codec == "linear16" else "raw",
    })

    def sanitize_config(incoming_voice: Optional[str], incoming_model: Optional[str]):
        """
        Sanitize incoming voice and model IDs to fallback to defaults appropriately.
        """
        ...
        valid_sarvam_models = ["bulbul:v2", "bulbul:v3-beta"]
        if incoming_model and "bulbul" not in incoming_model.lower():
            final_model = config.model
        elif incoming_model:
...
if __name__ == "__main__":
    ...
    host = os.getenv("TTS_SARVAM_HOST", "0.0.0.0")
    port = int(os.getenv("TTS_SARVAM_PORT", "8001"))
