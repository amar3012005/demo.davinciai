    ...
    for n in nodes:
        ...
        for raw_label, norm_label, mode, is_reduced in expanded_labels:
            if norm_label == blob:
                ...
                score = min(0.92, overlap / max(1, len(norm_label.split()))) if overlap > 0 else 0.0
            if score <= 0.0:
                continue
            if mode == "synonym":
                score -= 0.01
            if is_reduced:
                # Small precision penalty for reduced phrases while still allowing
                # high-confidence direct routing on strategic keyword subgoals.
                score -= 0.015
