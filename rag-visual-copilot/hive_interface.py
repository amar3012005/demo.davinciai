...
class HiveInterface:
    ...
    # ═══════════════════════════════════════════════════════════
    # QUICK PROBE — lightweight in-domain check for pre-decision gate
    # ═══════════════════════════════════════════════════════════

    _quick_probe_cache: Dict[str, Any] = {}
    _quick_probe_cache_ts: Dict[str, float] = {}
    _QUICK_PROBE_TTL = 3.0  # seconds
    ...
    async def quick_probe(
        ...
        goal: str,
        session_id: str = "",
        limit_strategy: int = 1,
        limit_hints: int = 5,
    ) -> Dict[str, Any]:
        """
        Lightweight in-domain Hive probe for the pre-decision gate.
        No cross-domain expansion. Returns strategy existence, top hints,
        and a probe confidence score.

        Cached for 3 seconds keyed by session_id + goal + domain.
        """
        cache_key = f"{session_id}:{goal}:{domain}"
        now = time.time()

        # Check TTL cache
        cached_ts = self._quick_probe_cache_ts.get(cache_key, 0.0)
        if now - cached_ts < self._QUICK_PROBE_TTL and cache_key in self._quick_probe_cache:
            logger.debug(f"quick_probe cache hit: {cache_key}")
            return self._quick_probe_cache[cache_key]
        ...
        if not domain or not self.qdrant or not QDRANT_AVAILABLE:
            self._quick_probe_cache[cache_key] = result
            self._quick_probe_cache_ts[cache_key] = now
            return result

        try:
            # 1. Check domain candidates: exact host first, then root domain fallback.
            ...
            for d in (domain, root_domain):
                if d and d not in domain_candidates:
                    domain_candidates.append(d)

            matched_domain = ""
            for d in domain_candidates:
                if await self.is_domain_indexed(d):
                    matched_domain = d
                    break

            result["domain_indexed"] = bool(matched_domain)
            result["matched_domain"] = matched_domain
            if not matched_domain:
                # Strict domain safety: never perform cross-domain fallback probes.
                self._quick_probe_cache[cache_key] = result
                self._quick_probe_cache_ts[cache_key] = now
                return result

            # 2. Quick strategy lookup
            query_text = f"{goal} on {matched_domain}"
            vector = await self._embed(query_text) if self.embeddings else None
            ...
            if vector:
                strategy_filter = Filter(
                    must=[
                        ...
                        FieldCondition(key="domain", match=MatchValue(value=matched_domain)),
                    ]
                )
                strategy_results = await self._query_qdrant(
                    vector=vector,
                ...
                if strategy_results:
                    point = strategy_results[0]
                    payload = point.get('payload', {}) if isinstance(point, dict) else getattr(point, 'payload', {})
                    score = point.get('score', 0.0) if isinstance(point, dict) else getattr(point, 'score', 0.0)
                    result["has_strategy"] = True
                    result["strategy_sequence"] = payload.get("sequence", [])
                    result["probe_confidence"] = max(result["probe_confidence"], score)

                # 3. Quick visual hints lookup
                hints_filter = Filter(
                    must=[
                        FieldCondition(key="domain", match=MatchValue(value=matched_domain)),
                    ]
                )
                hint_results = await self._query_qdrant(
                    vector=vector,
                    query_filter=hints_filter,
                    limit=limit_hints,
                    score_threshold=0.3,
                )
                for pt in (hint_results or []):
                    p = pt.get('payload', {}) if isinstance(pt, dict) else getattr(pt, 'payload', {})
                    s = pt.get('score', 0.0) if isinstance(pt, dict) else getattr(pt, 'score', 0.0)
                    selector = p.get('selector', '')
                    text_pattern = p.get("text_pattern")
                    if selector or text_pattern:
                        result["top_hints"].append({
                            "selector": selector,
                            "element_type": p.get("element_type", "link"),
                            "zone": p.get("zone", "main"),
                            "text_pattern": text_pattern,
                            "score": s,
                        })
                        result["probe_confidence"] = max(result["probe_confidence"], s * 0.8)
            else:
                logger.debug(
                    f"quick_probe vector unavailable for domain={matched_domain}; "
                    "using scroll-based fallback retrieval."
                )

            # 4. Scroll-based fallback retrieval when vector search did not provide enough signal.
            #    This guarantees pre-decision receives Hive strategy/hints if present in-domain.
            if (not result["has_strategy"]) or (not result["top_hints"]):
                try:
                    # Strategy fallback
                    if not result["has_strategy"]:
                        strategy_scroll = await self.qdrant.scroll(
                            collection_name=self.collection,
                            scroll_filter=Filter(
                                must=[
                                    FieldCondition(key="doc_type", match=MatchValue(value="Strategy_Sequence")),
                            ...
                            limit=max(1, limit_strategy),
                            with_payload=True,
                            with_vectors=False,
                        )
                        strategy_points = strategy_scroll[0] if isinstance(strategy_scroll, tuple) else strategy_scroll
                        if strategy_points:
                            p0 = strategy_points[0]
                            payload = p0.get('payload', {}) if isinstance(p0, dict) else getattr(p0, 'payload', {})
                            seq = payload.get("sequence") or payload.get("strategy_sequence") or []
                            if seq:
                                result["has_strategy"] = True
                                result["strategy_sequence"] = list(seq)
                                payload_conf = float(payload.get("confidence") or 0.55)
                                result["probe_confidence"] = max(result["probe_confidence"], payload_conf)

                    # Visual hints fallback
                    if not result["top_hints"]:
                        hints_scroll = await self.qdrant.scroll(
                            collection_name=self.collection,
                            scroll_filter=Filter(
                                must=[
                            ...
                            limit=max(limit_hints * 2, 8),
                            with_payload=True,
                            with_vectors=False,
                        )
                        hint_points = hints_scroll[0] if isinstance(hints_scroll, tuple) else hints_scroll
                        for pt in (hint_points or []):
                            payload = pt.get('payload', {}) if isinstance(pt, dict) else getattr(pt, 'payload', {})
                            selector = payload.get("selector", "")
                            text_pattern = payload.get("text_pattern") or payload.get("entity")
                            if not selector and not text_pattern:
                                continue
                            result["top_hints"].append({
                                "selector": selector,
                                "element_type": payload.get("element_type", "link"),
                            ...
                            if len(result["top_hints"]) >= limit_hints:
                                break
                        if result["top_hints"]:
                            best_hint_score = max(float(h.get("score") or 0.0) for h in result["top_hints"])
                            result["probe_confidence"] = max(result["probe_confidence"], best_hint_score * 0.8)
                except Exception as fallback_err:
                    logger.warning(f"quick_probe scroll fallback failed for domain={matched_domain}: {fallback_err}")

        except Exception as e:
            logger.warning(f"quick_probe failed for domain={domain}: {e}")

        logger.info(
            "quick_probe result: "
            f"domain={domain} matched={result.get('matched_domain') or 'none'} "
            f"indexed={result.get('domain_indexed')} "
            f"strategy={result.get('has_strategy')} "
            f"hints={len(result.get('top_hints') or [])} "
            f"conf={float(result.get('probe_confidence') or 0.0):.2f}"
        )
        self._quick_probe_cache[cache_key] = result
        self._quick_probe_cache_ts[cache_key] = now
        return result
