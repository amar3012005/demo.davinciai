...
6. **MODALS ARE OFTEN MENUS**: Before dismissing a modal or popup, CHECK if the data you need is inside it. If the goal is "select model X" and a "Select Model" modal is open, interact with the modal — do NOT dismiss it.

7. **STRICT: NO HALLUCINATED ANSWERS**:
   - NEVER put the answer to the user's question in a sub-goal description.
   - Example (BAD): "Answer that the price is $20/million tokens."
   - Example (GOOD): "Locate the specific pricing for 'model-x' on the page and answer the user."
   - You are a PLANNER, not a researcher. Your task is to plan actions to FIND the data, not guess it.

8. **success_signal for navigation steps**:
...
9. Keep it to 1-3 steps maximum.
...
{{
  "steps": [
    {{
      ...
      "type": "navigate|observe|interact|answer|dismiss_obstacle",
...
Set requires_reasoning=true for:
- Dropdown or select interactions (open + pick)
- Form fields that depend on dynamic content or validation
- Steps where the target may be behind an obstacle (modal, overlay)
- Multi-step interactions (expand accordion → find element)
Default: false for simple clicks and navigation."""
...
# ── v5.1: Navigator Prompts (8B Fast + 120B Deep) ──────────────────────────

NAVIGATOR_8B_PROMPT = """<system_configuration>
You are TARA, a Visual Co-Pilot. Pick the NEXT action.
Output ONLY valid JSON.
</system_configuration>
...
Targeting:
- Use target_id from ELEMENTS above. If no id, use click_coords: x=rect.x+width/2, y=rect.y+height/2.
- speech: Under 12 words. Say what you're DOING ("Heading to docs."), not what you see.

Respond in json format.

{{"action":{{"type":"{action_type}","target_id":"<id>","text":"<if typing/answering>"}},"confidence":"high|medium","speech":"<brief narration>"}}"""


NAVIGATOR_120B_PROMPT = """<system_configuration>
You are TARA, an autonomous web navigator handling a complex situation.
You must reason carefully, then pick ONE action.
</system_configuration>
...
<thinking_protocol>
Before deciding, reason through these steps:
1. INTENT: What does the user actually want? Personal data vs information vs visible data.
2. OBSTACLES: Is there a modal or overlay? CRITICAL: Modals are often MENUS or DATA PANELS.
   - If the modal contains data relevant to your mission (model list, usage stats, settings), LOOK INSIDE IT — do NOT dismiss it.
   - Only dismiss modals that are clearly blocking (cookie banners, login prompts, ads).
3. CANDIDATES: Which element from the page state best matches the current step?
4. FAILURES: Check self_corrections. What failed before? Try a DIFFERENT approach.
5. CONSTRAINT: My target_id MUST exist in the page_state above. If not visible, scroll.
</thinking_protocol>

<rules>
1. Pick ONE action. Cookie/privacy banners → dismiss. Content modals → look inside first.
2. For dropdowns: first "click" to open, then pick the option on next step.
3. For forms: fill the next empty field. Check form_section for field status.
4. target_id MUST exist in page_state. If no id, use click_coords: x=rect.x+width/2, y=rect.y+height/2.
5. speech: Describe YOUR ACTION briefly. For answers: summarize the data (2-3 sentences).
6. HITL: If sensitive data needed (password, payment), use type "user_input_required".
7. If >1 matching elements and you can't decide, use type "clarify" and ask the user.
</rules>

Respond in json format.

JSON:
{{"observation":"State: [page]. Gap: [analysis].","reasoning":"[structured reasoning following thinking_protocol]","action":{{"type":"click|type_text|scroll_to_view|answer|dismiss|clarify|user_input_required|none","target_id":"<id>","text":"<value if typing/answering>"}},"confidence":"high|medium|low","speech":"<brief narration>"}}"
"""
...
# Stagnation hints (Shared between _reason and _navigate_120b)
_STAGNATION_HINTS = {
...
class VisualOrchestrator:
    ...
    def __init__(self, groq_provider: GroqProvider, qdrant_client=None, embeddings=None, redis_client=None):
        ...
        # v5.1: Navigator models (fast 8B + deep reasoning)
        self.navigator_fast_model = os.getenv("NAVIGATOR_FAST_MODEL", "llama-3.1-8b-instant")
        self.navigator_deep_model = os.getenv("NAVIGATOR_DEEP_MODEL", "openai/gpt-oss-120b")
    ...
    # ── v5.1: Three-Layer Navigator (replaces single REASON call) ────────
    ...
    async def navigate_step(
        ...
        """
        v5.1 Decision Router: Detective → Route → Navigate.
        Replaces the single _reason() call with intelligent model routing.
        """
        t0 = time.time()
        ...
        logger.info(
            f"🔍 Detective: {report.complexity} | model={report.recommended_model} | "
            f"action={report.recommended_action} | obstacle={report.has_obstacle} | "
            f"candidates={len(report.candidates)} | {report.routing_reason}"
        )

        # ── Phase 2: Route to the right handler ──────────────────────
        if report.recommended_model == "shortcut":
            result = self._shortcut(report, session)
            ...
            # Per-step Qdrant context (only for 120B — worth the extra ~50ms)
            step_context = ""
            if self.use_hivemind and session.hivemind_mode == "mapped":
                try:
                    step_context = await self.get_step_context(dom_elements, url, session.client_id)
                except Exception:
                    step_context = ""
            result = await self._navigate_120b(report, session, goal, page_slice, step_context=step_context, stagnation_action=stagnation_action)
        ...
        # Null normalization (robustness from v1)
        action = result.get("action", {})
        action.setdefault("target_id", "")
        ...
        result.setdefault("reasoning", report.routing_reason)
        ...
        result["_detective"] = {
            ...
            "obstacle": report.has_obstacle,
            "page_type": report.page_type,
        }

        return result

    def _shortcut(self, report: 'DetectiveReport', session) -> dict:
        ...
        if report.recommended_action == "dismiss" and report.obstacle_dismiss_id:
            return {
                ...
                "confidence": "high",
                "speech": f"Dismissing {report.obstacle_type.replace('_', ' ')}.",
                "reasoning": f"Shortcut: dismiss {report.obstacle_type}",
            }

        if report.recommended_action == "answer" and report.evidence:
            # Truncate evidence for speech
            speech = report.evidence[:300]
            if len(report.evidence) > 300:
                speech = speech.rsplit('\n', 1)[0]
            return {
                "action": {"type": "answer", "target_id": "", "text": report.evidence[:500]},
                "confidence": "high",
                "speech": speech,
                "reasoning": "Shortcut: answer visible in DOM/tables",
            }
        ...
        # Fallback: scroll
        return {
            "action": {"type": "scroll", "target_id": "", "text": "down"},
            "confidence": "medium",
            "speech": "Scrolling down to find more options.",
            "reasoning": "Shortcut: no clear target, scrolling",
        }

    async def _navigate_8b(self, report: 'DetectiveReport', session, goal: str) -> dict:
        ...
        # Build action section from detective report
        action_section = self._build_8b_action_section(report)

        # Build history section (last 3 actions)
        history_section = self._build_history_section(session)

        prompt = NAVIGATOR_8B_PROMPT.format(
        ...
        try:
            response = await self.groq.generate(
                prompt,
                model=self.navigator_fast_model,
                temperature=0.1,
                max_tokens=256,
                response_format={"type": "json_object"},
            )
            ...
            data.setdefault("reasoning", f"8B pick from {len(report.candidates)} candidates")
            data.setdefault("observation", report.page_identity)
            return data
        except Exception as e:
            logger.error(f"Navigator 8B failed: {e}")
            # Fallback to shortcut
            return self._shortcut(report, session)

    async def _navigate_120b(self, report: 'DetectiveReport', session, goal: str, page_slice: str, step_context: str = "", stagnation_action: str = "continue") -> dict:
        ...
        # Inject per-step Qdrant context into page_slice if available
        if step_context:
            page_slice = f"{step_context}\n\n{page_slice}"
        ...
        if report.has_obstacle:
            obstacle_section = (
                ...
                f"DISMISS BUTTON: {report.obstacle_dismiss_id or 'NOT FOUND — look for Close/Accept/X'}\n"
        ...
        # Form info
        form_section = ""
        if report.page_type == "form" and report.form_fields:
            lines = ["<form_state>"]
            for f in report.form_fields[:6]:
                status = f"filled='{f['value']}'" if f['filled'] else "EMPTY"
                req = " [REQUIRED]" if f.get("required") else ""
                lines.append(f"  [{f['type']}] {f['label']} (id: {f['id']}) [{status}]{req}")
            if report.next_empty_field:
                lines.append(f"\nNEXT FIELD: {report.next_empty_field['label']} (id: {report.next_empty_field['id']})")
            lines.append("</form_state>")
            form_section = "\n".join(lines)

        # Reflexion memory
        reflexion = "(No prior failures — fresh attempt)"
        ...
        prompt = NAVIGATOR_120B_PROMPT.format(
            ...
            obstacle_section=obstacle_section or "(No obstacles detected)",
            form_section=form_section or "(Not a form page)",
            compressed_history=history,
            reflexion_memory=reflexion,
            detective_reasoning=report.routing_reason,
        ...
        try:
            result = await self.groq.generate_with_reasoning(
                prompt,
                model=self.navigator_deep_model,
            ...
            # Parse with multi-stage recovery
            data = self._parse_reason_json(content, cot)
            data["_ms"] = int((time.time() - t0) * 1000)
            data["_cot"] = cot[:500]
            return data
        except Exception as e:
            logger.error(f"Navigator 120B failed: {e}")
            # Fallback to 8B
            return await self._navigate_8b(report, session, goal)

    def _build_8b_action_section(self, report: 'DetectiveReport') -> str:
        """Build the action context section for the 8B navigator prompt."""
        if report.recommended_action == "answer" and report.evidence:
            return f"DATA FOUND (read this to the user):\n{report.evidence[:400]}"

        if report.recommended_action == "type" and report.next_empty_field:
            field = report.next_empty_field
            return f"FIELD: {field['label']} (id: {field['id']}, type: {field['type']})"
    ...
    def _build_history_section(self, session) -> str:
        """Build compact history section for 8B prompt."""
        if not session.action_ledger:
            return ""
        lines = ["RECENT ACTIONS (do NOT repeat):"]
        for record in session.action_ledger[-3:]:
            outcome = record.actual_outcome or "pending"
            target = record.target_text or record.target_id or ""
            lines.append(f"  - {record.action_type} on '{target}' → {outcome}")
        return "\n".join(lines)

    # ── v5: Single REASON call (kept as fallback) ──────────────────────────
        ...
        try:
            ...
            for step in raw_list:
                subgoals.append(SubGoal(
                    ...
                    requires_reasoning=bool(step.get("requires_reasoning", False)),
                    status="pending"
                ))
    ...
    async def plan_next_step(
        ...
        # 5. v5.1 Navigate Step (Detective → Route → Navigate)
        reason_res = await self.navigate_step(
            session=session,
            dom_elements=dom_context,
            ...
            data_tables=data_tables,
            page_title=page_title,
            last_action_id=session.action_ledger[-1].target_id if session.action_ledger else "",
        )
        ...
        routing = reason_res.get("_routing", "unknown")
        detective_info = reason_res.get("_detective", {})
        logger.info(
            f"⚡ v5.1 Turn {session.step_number} | "
            f"{total_time}ms total | "
            f"route={routing} ({reason_res.get('_ms', 0)}ms) | "
            f"detective={detective_info.get('complexity', '?')}/{detective_info.get('model', '?')} | "
        ...
        return {
            ...
            "_phase_timings": {
                "navigate_ms": reason_res.get("_ms", 0),
                "total_ms": total_time,
            },
            "_routing": routing,
            "_detective": detective_info,
            "_validation": {
    ...
    def _check_subgoal_advancement(self, session, page_state, dom_context: list, validation: 'ValidationResult' = None, current_url: str = ""):
        ...
        if validation and validation.success:
            ...
            # ── LOOP PREVENTION ──
            # If we navigated (URL changed) but the sub-goal is NOT advancing (because it didn't match specific keywords),
