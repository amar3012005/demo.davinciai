...
class ConfigLoader:
    ...
    def _parse_config(self, data: Dict[str, Any]) -> OrchestratorConfig:
        ...
        if "organization" in data:
            ...
            config.organization = OrganizationConfig(
                ...
                tenant_id=os.getenv("TENANT_ID", org_data.get("tenant_id", "tenant")),
                # Integrations
        ...
        if "services" in data:
            ...
            if "stt" in services_data:
                ...
                stt_url = os.getenv("STT_SERVICE_URL", stt_data.get("url", "http://stt:8002"))
            ...
            if "tts" in services_data:
                ...
                config.services.tts = TTSConfig(
                    url=os.getenv("TTS_SERVICE_URL", tts_data.get("url", "http://tts:8000")),
            ...
            if "rag" in services_data:
                ...
                config.services.rag = RAGConfig(
                    ...
                    language=os.getenv("RAG_LANGUAGE", rag_data.get("language", "english")),
        ...
        if "dialogue" in data:
            ...
            self._apply_dialogue_placeholders(config)
    ...
    def _apply_dialogue_placeholders(self, config: OrchestratorConfig):
        """Resolve template placeholders in dialogue content using loaded identity."""
        replacements = {
            "ORGANIZATION_NAME": config.organization.name,
            "ORGANIZATION_FULL_NAME": config.organization.full_name,
            "AGENT_NAME": config.agent.name,
            "AGENT_ID": config.agent.id,
            "TENANT_ID": config.agent.tenant_id,
        }
        config.dialogue = self._replace_placeholders(config.dialogue, replacements)

    def _replace_placeholders(self, value: Any, replacements: Dict[str, str]) -> Any:
        """Recursively replace placeholders in nested dict/list/string values."""
        if isinstance(value, dict):
            return {k: self._replace_placeholders(v, replacements) for k, v in value.items()}
        if isinstance(value, list):
            return [self._replace_placeholders(item, replacements) for item in value]
        if isinstance(value, str):
            out = value
            for key, replacement in replacements.items():
                out = out.replace(f"{{{{{key}}}}}", str(replacement or ""))
            return out
        return value
