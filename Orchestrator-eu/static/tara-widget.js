 ...
 * WebSocket: configured dynamically at runtime
 * 
 * Flow:
...
(function () {
    ...
    const getEnvConfig = () => {
        ...
        if (window.TARA_FORCE_LOCAL && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')) {
            ...
            console.log('🔒 [Widget] FORCED localhost URL:', localUrl);
        ...
        // Check for localhost development (automatic detection) - ALWAYS default to local
        ...
        // Default to localhost for development (NOT production)
        const port = window.location.port || '8004';
        const defaultLocal = `ws://localhost:${port}/ws`;
        console.log('🔧 [Widget] Defaulting to localhost:', defaultLocal);
    ...
    const DEFAULTS = {
        wsUrl: ENV_CONFIG.wsUrl || 'ws://localhost:8004/ws', // Default LOCAL (not production)
    ...
    // Orb image defaults to same-origin static path, overridable by TARA_ASSET_BASE_URL
      ...
      `;
        ...
        async startVisualCopilot(resumeSessionId = null, mode = 'interactive') {
            try {
                ...
                // 4. Initialize TaraSensor for delta streaming (ULTIMATE TARA)
                console.log('👁️ Initializing TaraSensor...');
                if (window.TaraSensor) {
                    this.sensor = new window.TaraSensor(this.ws, {
                        sendFullScanOnInit: true,
                        debounceMs: 150,
                        maxBatchSize: 50
                    });
                    this.sensor.start();
                    console.log('✅ TaraSensor initialized - using delta streaming');
                } else {
                    // Fallback to legacy scan
                    console.log('⚠️ TaraSensor not available, using legacy scan');
        ...
        connectWebSocket() {
            return new Promise((resolve, reject) => {
                ...
                this.ws.onopen = () => {
                    ...
                    // ── PHOENIX WAKE-UP: Check URL for transferred session ──
                    if (window.location.hash.includes('tara_phoenix=')) {
                        try {
                            const encodedData = window.location.hash.split('tara_phoenix=')[1];
                            const phoenixData = JSON.parse(atob(encodedData));

                            console.log('🔥 Phoenix Protocol: Restoring cross-domain session', phoenixData.sid);

                            // Restore session ID
                            sessionStorage.setItem('tara_session_id', phoenixData.sid);
                            localStorage.setItem('tara_session_id', phoenixData.sid);

                            // Clean up the URL instantly so the user doesn't see the ugly hash
                            history.replaceState(null, "", window.location.pathname + window.location.search);

                            // Ask backend to reconnect and replay recent history
                            this.ws.send(JSON.stringify({
                                type: 'request_history',
                                session_id: phoenixData.sid,
                                mission_goal: phoenixData.goal || ''
                            }));
                        } catch (e) {
                            console.error('Phoenix wake-up failed:', e);
                        }
                    }
        ...
        scanPageBlueprint(force = false) {
            ...
            allElements.forEach(el => {
                ...
                // Skip TARA's own UI so the agent never sees/clicks
                // her own buttons ("Interactive Mode", "Turbo Mode", etc.)
                // ══════════════════════════════════════════════════
                ...
                elements.push({
                    id: finalId,
                    tag: el.tagName.toLowerCase(),   // RAW HTML tag — used by live_graph get_links/get_buttons
                    text: cleanText,
                    type: type,                      // Semantic type (may be 'header' for h1-h6)
        ...
        async handleBackendMessage(msg) {
            ...
            if (msg.type === 'asset_data') {
                ...
                // ── PHOENIX PROTOCOL: Backend History Replay ──
                // Authoritative history from Redis — merge with pre-filled messages.
                // Clear what we pre-filled and replace with the real thing.
                if (msg.turns && msg.turns.length > 0) {
                    if (this.chatMessages) this.chatMessages.innerHTML = '';
                    msg.turns.forEach(turn => {
                        this.appendChatMessage(turn.text, turn.role === 'user' ? 'user' : 'ai', false);
                    });
                    console.log(`🔥 Phoenix: Backend restored ${msg.turns.length} history turns`);
                }
                // Resume the mission if goal was passed back
                if (msg.mission_goal) {
                    this._currentMissionGoal = msg.mission_goal;
                ...
                try {
                    ...
                    if (targetUrl.origin === window.location.origin) {
                        ...
                        // ── PHOENIX PROTOCOL: State Handoff ──
                        if (msg.phoenix) {
                            try {
                                const sessionId = sessionStorage.getItem('tara_session_id') ||
                                    localStorage.getItem('tara_session_id');

                                // 🌟 PHOENIX BRIDGE: Pass session via URL hash to survive Cross-Domain
                                const targetUrl = new URL(msg.url, window.location.origin);
                                const phoenixData = {
                                    sid: sessionId || msg.session_id,
                                    goal: this._currentMissionGoal || msg.mission_goal || ""
                                };

                                // Use base64 to keep the URL relatively clean
                                targetUrl.hash = `tara_phoenix=${btoa(JSON.stringify(phoenixData))}`;
                                window.location.href = targetUrl.toString();
                            } catch (err) {
                                console.warn('⚠️ Phoenix: Failed to format URL hash', err);
                                window.location.href = msg.url;
                            }
                        } else {
                            window.location.href = msg.url;
            ...
            try {
                if (type === 'wait') {
                    ...
                    if (el) {
                        ...
                        // 🔒 LOCK: Stop asking for plans while navigating
                        this.isNavigating = true;

                        // Give the browser 2 seconds to change pages before 
                        // allowing the next plan_next_step request (which fires via execution_complete)
