...
(function () {
    ...
    const getEnvConfig = () => {
        ...
        const port = window.location.port || '8004';
        const fallbackWs = window.location.protocol === 'https:'
            ? `wss://${window.location.hostname}:${port}/ws`
            : `ws://${window.location.hostname}:${port}/ws`;
        console.log('🔧 [Widget] Using same-host fallback URL:', fallbackWs);
        return { wsUrl: fallbackWs };
    };
